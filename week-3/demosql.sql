...
WHERE exists (SELECT *  
              FROM employeestate 
              WHERE employeestate.DEPT_ID = department.DEPT_ID); 
...
INSERT INTO COURSES VALUES(1, 'BTECH');
INSERT INTO COURSES VALUES(2, 'MTECH');
INSERT INTO COURSES VALUES(3, 'MBA');
SELECT * FROM COURSES;
...
WHERE EXISTS (SELECT COUR_ID 
              FROM COURSES 
              WHERE COUR_NAME IN ('MBA'));
SELECT Emp_Name 
FROM personal 
WHERE EXISTS (SELECT COUR_ID 
              FROM COURSES 
              WHERE COUR_NAME IN ('BBA'));
-- NOT EXISTS OPERATOR
SELECT Emp_Name 
FROM personal 
WHERE NOT EXISTS (SELECT COUR_ID 
              FROM COURSES 
              WHERE COUR_NAME IN ('MBA'));
SELECT Emp_Name 
FROM personal 
...
--  Numeric Functions
SELECT ROUND(5.5567, 2);
SELECT ABS(-5.5);
SELECT CEILING(5.5);
SELECT FLOOR(5.5);
SELECT TRUNCATE(5.537, 1);
SELECT EXP(3);
SELECT GREATEST(7,4,7,8); 
SELECT LEAST(7,4,7,8);   
SELECT LOG(45);    
SELECT MOD(29,3);
SELECT PI();
SELECT POWER(3,3);
...
SELECT DATE_SUB(NOW(), INTERVAL -1 DAY);
SELECT DATEDIFF(NOW(), '2023-03-1');
SELECT TIME_TO_SEC(NOW() - TIME_TO_SEC('13:40'));


-- IF STATEMENT
USE employee;
SELECT Emp_ID, Emp_Name, PERCENTAGE,
IF(PERCENTAGE >= 47, "PASS", "FAIL") AS RESULT
FROM personal;

-- CASE STATEMENT
SELECT Emp_ID, Emp_Name, PERCENTAGE,
CASE
   WHEN PERCENTAGE >= 65 AND PERCENTAGE <=75 THEN "FIRST CLASS"
   WHEN PERCENTAGE >= 55 AND PERCENTAGE <=65 THEN "SECOND CLASS"
   WHEN PERCENTAGE >= 47  THEN "FAIL"
END AS RESULT
FROM personal;
